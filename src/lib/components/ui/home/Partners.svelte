                ...
                <Carousel.Content class="flex flex-nowrap md:grid md:grid-cols-4 lg:grid-cols-8 gap-6">
                    {#each Object.values(PARTNERS) as partner}
                        <Carousel.Item class="basis-auto md:basis-1/4 lg:basis-1/8 flex justify-center">
                            <div class="flex justify-center items-center hover:text-gray-900 dark:hover:text-white">
                                <img src={partner.logo} alt={partner.title} class="h-14 max-h-14 w-auto object-contain p-2 rounded-lg" />
                            </div>
                        </Carousel.Item>
                    {/each}
                </Carousel.Content>
