<script>
    ...
    import { toastMessage, toastType, hideToast, TOAST_POSITION, toastPosition } from '$lib/stores/toastStore';
    let type = 'success';
    const toastConfig = {
        success: {
    ...
    $: positionClass = () => {
        ...
        if($toastPosition === TOAST_POSITION.TOP_CENTER) return 'top-5 left-1/2 transform -translate-x-1/2';
        ...
        if($toastPosition === TOAST_POSITION.BOTTOM_CENTER) return 'bottom-5 left-1/2 transform -translate-x-1/2';
        return '';
    }
</script>
...
{#if $toastMessage}
    <div transition:fade>
		<div
			id="toast-{type}"
			class={`z-100 fixed flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800 ${positionClass()}`}
			role="alert"
		>
			<div
