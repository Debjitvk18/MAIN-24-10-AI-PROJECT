<script lang="ts">
    ...
    async function updatePassword(event: Event) {
        ...
        try {
            const data = await userService.updatePassword({ password, password_confirmation, current_password });
            if (data.success) {
                savedShown = true;
                setTimeout(() => {
                    savedShown = false;
            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form on:submit={handleSubmit} enctype="multipart/form-data">
                    <div class="px-4 py-5 bg-white sm:p-6 shadow sm:rounded-tl-md sm:rounded-tr-md">
                        ...
                        <div class="grid grid-cols-6 gap-6">
                            ...
                            <div class="col-span-6 sm:col-span-4">
                                ...
                                <input type="file" class="hidden" bind:this={photoInput} on:change={handleFileChange}>
                                ...
                                <button type="button" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:text-gray-500 focus:outline-none focus:border-blue-300 focus:ring focus:ring-blue-200 active:text-gray-800 active:bg-gray-50 disabled:opacity-25 transition mt-2 mr-2" on:click={() => photoInput.click()}>
                                    Select A New Photo
                                </button>
                
                                <button type="button" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:text-gray-500 focus:outline-none focus:border-blue-300 focus:ring focus:ring-blue-200 active:text-gray-800 active:bg-gray-50 disabled:opacity-25 transition mt-2" on:click={deleteProfilePhoto}>
                                    Remove Photo
                                </button>
                            </div>
                            ...
                            <div class="col-span-6 sm:col-span-4">
                                ...
                                <input class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full" id="name" type="text" bind:value={name} autocomplete="name">
                            </div>
                
                            <!-- Email -->
                            <div class="col-span-6 sm:col-span-4">
                                <label class="block font-medium text-sm text-gray-700" for="email">Email</label>
                                <input class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full" id="email" type="email" bind:value={email}>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end px-4 py-3 bg-gray-50 text-right sm:px-6 shadow sm:rounded-bl-md sm:rounded-br-md">
                        ...
                        {:else}
                            {#if shown}
                                <div class="text-sm text-gray-600 mr-3" transition:fade={{ duration: 1500 }}>Saved.</div>
                            {/if}
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6 shadow sm:rounded-tl-md sm:rounded-tr-md">
                            ...
                            <div class="grid grid-cols-6 gap-6">
                                <div class="col-span-6 sm:col-span-4">
                                    ...
                                    <input class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full" id="current_password" type="password" autocomplete="current-password" bind:value={current_password}>
                                </div>

                                <div class="col-span-6 sm:col-span-4">
                                    <label class="block font-medium text-sm text-gray-700" for="password">New Password</label>
                                    <input class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full" id="password" type="password" autocomplete="new-password" bind:value={password}>
                                </div>

                                <div class="col-span-6 sm:col-span-4">
                                    <label class="block font-medium text-sm text-gray-700" for="password_confirmation">Confirm Password</label>
                                    <input class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full" id="password_confirmation" type="password" autocomplete="new-password" bind:value={password_confirmation}>
                                </div>
                            </div>
                        </div>
