...
<div class="container max-w-100">
	<div class="flex-1 space-y-4">
		...
		<div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
			...
			<Card.Root>
				<Card.Header class="flex flex-row items-center justify-between space-y-0 pb-2">
					<Card.Title class="text-sm font-medium">Subscriptions 
				...
				<Card.Content>
					{#if $user.subscription}
						<div class="text-2xl font-bold">{$user.subscription.currency_symbol}{$user.subscription.charge}</div>
						<p class="text-muted-foreground text-xs">Next billing date: {$user.next_billing_date ?? 'N/A'}</p>
					{:else}
		...
		{:else}
			{#each Array(4) as _}
				<Card.Root>
					<Card.Header class="flex flex-row items-center justify-between space-y-0 pb-2">
						<Card.Title class="text-sm font-medium bg-gray-300 animate-pulse h-4 w-1/3 rounded"></Card.Title>
